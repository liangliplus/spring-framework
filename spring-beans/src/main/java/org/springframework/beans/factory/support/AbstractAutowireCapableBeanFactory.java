...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		// 判断需要创建的Bean 是否可以实例化， 即是否可以通过当前类加载器加载
		...
		try {
			...
			// 如果Bean 配置勒初始化前和初始化后的处理器(InstantiationAwareBeanPostProcessor,在方法我们可以自己实例化bean)，
			// 则试图返回一个需要创建Bean的代理对象
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		if (earlySingletonExposure) {
			...
			if (earlySingletonReference != null) {
				...
				//当bean依赖其他bean，并且当循环引用时不允许创建新实例对象
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		if (resolved) {
			//autowireNecessary 配置自动装配属性，使用容器自动装配实例化（就是使用有参的构造函数初始化）
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		//获取容器在解析Bean定义是为BeanDefinition 中设置的属性值
		PropertyValues pvs = (mbd.hasPropertyValues() ? mbd.getPropertyValues() : null);

		//获取自动装配的模式
		int resolvedAutowireMode = mbd.getResolvedAutowireMode();
		//（重要）自动装配的实现（优先级比显示依赖注入高，先执行自动装配）
	...
	protected void autowireByType(
		...
		//对Bean 对象中非简单属性（例如：不是简单继承对象，不是基本数据类型，字符，url等简单属性）
		String[] propertyNames = unsatisfiedNonSimpleProperties(mbd, bw);
		for (String propertyName : propertyNames) {
			try {
				//获取指定属性名称的属性描述器
				...
				if (Object.class != pd.getPropertyType()) {
					...
					for (String autowiredBeanName : autowiredBeanNames) {
						...
						// 真正实现注入的是 DefaultSingletonBeanRegistry.registerDependentBean
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		if (pvs instanceof MutablePropertyValues) {
			...
			// MutablePropertyValues converted 默认为false， 该字段含义为是否已经转换过
			if (mpvs.isConverted()) {
		...
		//创建一个Bean定义属性解析器，将Bean定义中的属性值解析为Bean 实例对象的实际值
		...
		for (PropertyValue pv : original) {
			...
			//属性需要转换
				...
				// （相对重要）value解析器（属性为对其他对象引用，先要解析属性值，然后对解析值进行依赖注入）
				Object resolvedValue = valueResolver.resolveValueIfNecessary(pv, originalValue);
				// 转换之后的属性值
				Object convertedValue = resolvedValue;
				// 属性是否可转换（必须有可写方法，并且不是嵌套的属性，例如user.name ）
				...
				//属性可转换，且属性原始值是字符串类型，且属性的原始类型值不是动态生成的字符串
				// 且属性的原始值不是集合或者数据类型
